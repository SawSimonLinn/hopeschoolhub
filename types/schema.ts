...
export const studentSchema = z.object({
  ...
  grade: z.preprocess(
    ...
    z.number({ invalid_type_error: "Grade must be a number." }).min(0, { message: "Grade cannot be negative." })
  ),
  age: z.preprocess(
    (val) => (val === "" || val === null || val === undefined ? undefined : Number(val)),
    z.number({ invalid_type_error: "Age must be a number." }).min(1, { message: "Age must be a positive number." })
  ),
  paymentType: z.enum(['Monthly', 'Yearly'], { required_error: "Payment type is required." }),
  amountPaid: z.preprocess(
    (val) => (val === "" || val === null || val === undefined ? undefined : Number(val)),
    z.number({ invalid_type_error: "Amount must be a number." }).min(0, { message: "Amount paid cannot be negative." })
  ),
  ...
  yearsOfEnroll: z.preprocess(
    (val) => (val === "" || val === null || val === undefined ? undefined : Number(val)),
    z.number({ invalid_type_error: "Years of enrollment must be a number." }).min(0, { message: "Years of enrollment cannot be negative." })
  ),
  ...
  contactNumber: z.string().min(7, { message: "Contact number must be at least 7 digits." }).regex(/^\+?[0-9\s-]+$/, { message: "Invalid contact number format."}),
  churchName: z.string().min(1, {message: "Church name is required."}).or(z.literal("").transform(() => "N/A")),
