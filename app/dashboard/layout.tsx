...
const ADMIN_DEFAULT_AVATAR =
  'https://avatars.githubusercontent.com/u/150866883?v=4';
const GUEST_DEFAULT_AVATAR =
...
export default function DashboardLayout({
  ...
  const mainNavItems = allNavItemsConfig.filter(
    item => item.main && (!item.adminOnly || isAdmin)
  );
  const settingsNavItem = allNavItemsConfig.find(
    item =>
      !item.main &&
    ...
    <>
      <SidebarHeader className='p-4'>
        <Link href='/dashboard' className='flex items-center gap-2'>
          ...
          <span className='text-xl font-headline font-semibold text-foreground group-data-[collapsible=icon]:hidden'>
            HopeSchoolHub
          </span>
        </Link>
      </SidebarHeader>
  ...
  const currentNavLink = allNavItemsConfig.find(
    item =>
      (pathname === item.href ||
                ...
                <DropdownMenuLabel className='font-normal'>
                  <div className='flex flex-col space-y-1'>
                    <p className='text-sm font-medium leading-none'>
                      {isGuest ? 'Guest User' : currentUser || 'Admin User'}
                    </p>
                    {!isGuest && (
