...
import { getTeacherById, deleteTeacher as deleteTeacherService } from "@/lib/dataService";
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter } from "@/components/ui/card";
import { ArrowLeft, Edit3, Trash2, CalendarDays, Mail, User, BookOpen, Hash, Briefcase, Star } from "lucide-react";
...
export default function TeacherDetailsPage() {
  ...
  const teacherId = Number(params.id as string);
   ...
   if (isLoading) {
    return (
       <div className="space-y-6">
        <PageHeader title="Loading Teacher Details..." description="Please wait.">
  ...
  const detailItem = (icon: React.ElementType, label: string, value?: string | number | null, isBadge: boolean = false, badgeVariant: "default" | "secondary" | "destructive" | "outline" = "secondary", guestSensitive: boolean = false) => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={teacher.name} description={`Teacher ID: ${teacher.teacherId}`}>
      ...
      <Card className="shadow-xl">
        <CardHeader>
          <CardTitle className="font-headline text-2xl">Teacher Profile</CardTitle>
          <CardDescription>All recorded details for {teacher.name}.</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-2">
          ...
          {detailItem(Mail, "Email Address", teacher.email || "N/A", false, "secondary", true)}
          ...
          {detailItem(CalendarDays, "Hire Date", teacher.hireDate && isValid(parseISO(teacher.hireDate)) ? format(parseISO(teacher.hireDate), "MMMM d, yyyy") : 'N/A')}
        </CardContent>
        {!isGuest && (
