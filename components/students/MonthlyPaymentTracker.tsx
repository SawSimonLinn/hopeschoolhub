...
export function MonthlyPaymentTracker({ student, onUpdate, isGuest }: MonthlyPaymentTrackerProps) {
    const [updatingMonth, setUpdatingMonth] = useState<number | null>(null);
    
    const handleUpdateClick = async (monthIndex: number, currentStatus: 'Paid' | 'Unpaid') => {
        if (isGuest) return;
            ...
            <CardContent>
                <ul className="space-y-3">
                    {student.monthlyPayments?.map((payment, index) => (
                       <li key={index} className="flex items-center justify-between p-2 rounded-md transition-colors even:bg-muted/30 hover:bg-muted/50">
                           ...
                           <div className="flex items-center gap-2">
                                <Badge variant={payment.status === 'Paid' ? 'default' : 'secondary'} className={payment.status === 'Paid' ? 'bg-green-500/90 text-white' : ''}>
                                    {payment.status}
                                </Badge>
                                {!isGuest && (
                                    <Button 
                                        ...
                                        aria-label={`Mark ${payment.month} as ${payment.status === 'Paid' ? 'Unpaid' : 'Paid'}`}
                                    >
                                        {updatingMonth === index ? (
